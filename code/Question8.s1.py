#Question 8 : La méthode du rejet en testant si X est dans B avant de tester s'il est dans A
import numpy as np
...
def X_T(lb):  # fonction qui donne une valeur aléatoire de X_T
    X=[random(),random()]
    while(estDansB(X,lb)!=1 and estDansA(X,lb)!=1):
        X=[random(),random()]
    return X
...
def Simulation(lb,nbs):
    Y2=[Gamma_CF(lb) for i in range(nbs+1)]
    nbs, bins, patches = plt.hist(Y2, num_bins, normed=1, facecolor='mediumslateblue', alpha=0.5,label=r'Simulation de $X_T$')
